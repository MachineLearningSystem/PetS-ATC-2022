...
int bench(int m, int n){
    ...
    half *A_d, *B_d, *C_d;
    CHECK_CUDA(cudaMalloc(&A_d, m * n * sizeof(half)));
    CHECK_CUDA(cudaMalloc(&B_d, n * sizeof(half)));
    CHECK_CUDA(cudaMalloc(&C_d, m * sizeof(float)));
    CHECK_CUDA(cudaMemcpy(A_d, A.data(), m * n * sizeof(half), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(B_d, B.data(), n * sizeof(half), cudaMemcpyHostToDevice));
    ...
    CHECK_CUSPARSE(cusparseCreateDnMat(&matA_dense,
        m, n, n, A_d, CUDA_R_16F, CUSPARSE_ORDER_ROW));
    CHECK_CUSPARSE(cusparseCreateDnVec(&vecB, n, B_d, CUDA_R_16F));
    ...
    CHECK_CUDA(cudaMalloc(&csrData, nnz * sizeof(half)));
    ...
    std::vector<half> C(m);
    CHECK_CUDA(cudaMemcpy(C.data(), C_d, m * sizeof(half), cudaMemcpyDeviceToHost));
