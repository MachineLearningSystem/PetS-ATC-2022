...
int main(int argc, char const *argv[]){
    ...
    random_init<float>(A.data(), A.size(), 0);
    random_init<float>(B.data(), B.size(), sparsity);

    device_vector<float> A_cuda(A);
    device_vector<float> B_cuda(B);
    device_vector<float> C_cuda(m * n);
    float* ptr_A = raw_pointer_cast(A_cuda.data());
    float* ptr_B = raw_pointer_cast(B_cuda.data());
    float* ptr_C = raw_pointer_cast(C_cuda.data());
    // gemm
    float alpha = 1.0f;
    float beta = 0.0f;
    bool transA_ = false;
    ...
    cublasGemmEx(handle, transB, transA, n, m, k, &alpha, ptr_B, CUDA_R_32F, ldb,
            ptr_A, CUDA_R_32F, lda, &beta, ptr_C, CUDA_R_32F, ldc, CUDA_R_32F, CUBLAS_GEMM_DEFAULT);
    ...
    host_vector<float> C(C_cuda);
    // print_matrix<float>(A.data(), m, k);
    // print_matrix<float>(B.data(), k, n);
    ...
    CHECK_CUDA( cudaMalloc((void**) &csrVal,  nnz * sizeof(float)) )
    ...
    host_vector<float> C_sparse(C_cuda);
    print_matrix<float>(C_sparse.data(), 1, 10);
